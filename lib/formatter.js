...
var _default = formatter = function (doc, topNode, language) {
...
var convertToText = function (doc, topNode) {
  let txts = [];
  const nodes = topNode.contents(); // To hold any text fragments that end up in text nodes outside of
  // html elements
  ...
  nodes.each(function () {
    ...
    const nodeName = node[0].name; // Handle top level text nodes by adding them to a running list
    // and then treating all the hanging nodes as one paragraph tag

    if (nodeType === "text") {
      hangingText += node.text(); // Same as 'continue'
  ...
  txts = _lodash.default.map(txts, txt => txt.trim()); // Make sure each text chunk includes at least one text character or number.
  ...
  const regex = (0, _xregexp.XRegExp)('[\\p{Number}\\p{Letter}]');
  txts = _lodash.default.filter(txts, txt => regex.test(txt));
  return txts.join('\n\n');
};
...
var removeFewwordsParagraphs = function (doc, topNode, language) {
  ...
  return allNodes.each(function () {
    ...
    const tag = el[0].name;
    const text = el.text();
    const stopWords = (0, _stopwords.default)(text, language);
