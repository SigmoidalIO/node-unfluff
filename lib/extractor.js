...
var _stopwords = _interopRequireDefault(require("./stopwords"));
...
meta[name='creator']`);
    ...
    if (authorList.length === 0) {
      const fallbackAuthor = __guard__(doc("span[class*='author']").first(), x => x.text()) || __guard__(doc("p[class*='author']").first(), x1 => x1.text()) || __guard__(doc("div[class*='author']").first(), x2 => x2.text()) || __guard__(doc("span[class*='byline']").first(), x3 => x3.text()) || __guard__(doc("p[class*='byline']").first(), x4 => x4.text()) || __guard__(doc("div[class*='byline']").first(), x5 => x5.text());
...
meta[name='DC.Publisher']`);
  ...
  text(doc, topNode, lang) {
    if (topNode) {
      ...
      return (0, _formatter.default)(doc, topNode, lang);
    } else {
      return "";
    }
  },
...
meta[name='twitter:image0']`);
  ...
  videos(doc, topNode) {
    ...
    _lodash.default.each(videoList, function (vid) {
  ...
  tags(doc) {
    ...
    return _lodash.default.uniq(tags);
  },
  ...
  calculateBestNode(doc, lang) {
    ...
    let i = 0;
    const parentNodes = [];
    const nodesWithText = []; // Walk all the p, pre and td nodes

    nodesToCheck.each(function () {
      const node = doc(this);
      const textNode = node.text();
      const wordStats = (0, _stopwords.default)(textNode, lang);
      const highLinkDensity = isHighlinkDensity(doc, node); // If a node contains multiple common words and isn't just a bunch
    ...
    const bottomNegativescoreNodes = nodesNumber * 0.25; // Walk all the potentially 'texty' nodes

    _lodash.default.each(nodesWithText, function (node) {
      let boostScore = 0.0; // If this node has nearby nodes that contain
      ...
      const textNode = node.text();
      const wordStats = (0, _stopwords.default)(textNode, lang);
      const upscore = Math.floor(wordStats.stopwordCount + boostScore); // Propigate the score upwards
    ...
    let topNodeScore = 0; // Walk each parent and parent-parent and find the one that
    // contains the highest sum score of 'texty' child nodes.
    // That's probably out best node!

    _lodash.default.each(parentNodes, function (e) {
...
const biggestTitleChunk = function (title, splitter) {
  ...
  const titlePieces = title.split(splitter); // find the largest substring

  _lodash.default.each(titlePieces, function (piece, i) {
...
var isBoostable = function (doc, node, lang) {
  ...
  nodes.each(function () {
    ...
    if (currentNodeTag === "p") {
      ...
      const wordStats = (0, _stopwords.default)(paraText, lang); // Check if the node contains more than 5 common words
...
const addSiblings = function (doc, topNode, lang) {
  ...
  sibs.each(function () {
    ...
    return _lodash.default.each(ps, p => topNode.prepend(`<p>${p}</p>`));
  });
  return topNode;
};
  ...
  if (currentSibling[0].name === 'p' && currentSibling.text().length > 0) {
    ...
    if (potentialParagraphs === null) {
      ...
      potentialParagraphs.each(function () {
        ...
        if (txt.length > 0) {
          const wordStats = (0, _stopwords.default)(txt, lang);
...
var cleanTitle = function (title, delimiters) {
  ...
  _lodash.default.each(delimiters, function (c) {
...
var rawTitle = function (doc) {
  let gotTitle = false;
  let titleText = ""; // The first h1 or h2 is a useful fallback

  _lodash.default.each([__guard__(__guard__(doc("meta[property='og:title']"), x1 => x1.first()), x => x.attr("content")), __guard__(__guard__(doc("h1[class*='title']"), x3 => x3.first()), x2 => x2.text()), __guard__(__guard__(doc("title"), x5 => x5.first()), x4 => x4.text()), __guard__(__guard__(doc("h1"), x7 => x7.first()), x6 => x6.text()), __guard__(__guard__(doc("h2"), x9 => x9.first()), x8 => x8.text())], function (candidate) {
