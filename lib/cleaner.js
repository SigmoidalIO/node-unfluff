...
var _default = cleaner = function (doc) {
...
const getReplacementNodes = function (doc, div) {
  ...
  childs.each(function () {
    const kid = doc(this); // node is a p
    ...
    if (kid[0].name === 'p' && replacementText.length > 0) {
      ...
      replacementText = [];
      return nodesToReturn.push(doc(kid).html()); // node is a text node
  ...
  _lodash.default.each(nodesToRemove, n => doc(n).remove());
...
var divToPara = function (doc, domType) {
  ...
  return divs.each(function () {
    ...
    if (items.length === 0) {
      ...
      _lodash.default.each(replaceNodes, function (node) {
        if (node !== '') {
